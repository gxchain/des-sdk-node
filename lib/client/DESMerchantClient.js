            ...
            this.getProduct(productId).then(prod => {
                try {
                    ...
                    prod.onlineDatasources.forEach(datasource => {
                        ...
                        let sigBytes = signature.toBuffer();
                        console.log('isConancial', !(sigBytes[1] & 0x80)
                            && !(sigBytes[1] == 0 && !(sigBytes[2] & 0x80))
                            && !(sigBytes[33] & 0x80)
                            && !(sigBytes[33] == 0 && !(sigBytes[34] & 0x80)), ops.signed_proxy_transfer_params.toBuffer(operation).toString('hex'));
                        let nonce = TransactionHelper.unique_nonce_uint64();
                        console.log(datasource.publicKey);
                        reqBody.push({
                    ...
                    console.log(JSON.stringify(reqBody, '  ', 2));
